...
class Py4jInterpreter(object):
    ...
    class Java:
        implements = ["de.invesdwin.scripting.python.runtime.py4j.pool.internal.IPy4jInterpreter"]
...
def callback(methodName, *parameters):
    if 'py4jScriptTaskCallbackContext' not in globals():
        if 'py4jScriptTaskCallbackContextUuid' in locals() or 'py4jScriptTaskCallbackContextUuid' in globals():
            ...
            py4jScriptTaskCallbackContext = gateway.jvm.de.invesdwin.scripting.python.runtime.py4j.Py4jScriptTaskCallbackContext.getContext(py4jScriptTaskCallbackContextUuid)
        else:
