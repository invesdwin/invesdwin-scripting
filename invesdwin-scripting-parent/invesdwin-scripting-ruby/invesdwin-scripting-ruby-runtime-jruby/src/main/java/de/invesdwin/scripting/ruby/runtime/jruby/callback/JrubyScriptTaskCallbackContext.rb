...
def callback(methodName, *parameters)
    if($jrubyScriptTaskCallbackContext.nil?)
        if($binding.containsKey("jrubyScriptTaskCallbackContextUuid"))
        	$jrubyScriptTaskCallbackContext = Java::DeInvesdwinScriptingRubyRuntimeJrubyCallback::JrubyScriptTaskCallbackContext.getContext($binding.get("jrubyScriptTaskCallbackContextUuid"))
        else
