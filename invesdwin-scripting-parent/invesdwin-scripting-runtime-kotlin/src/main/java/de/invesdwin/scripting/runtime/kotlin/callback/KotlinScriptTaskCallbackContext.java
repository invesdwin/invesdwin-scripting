package de.invesdwin.scripting.runtime.kotlin.callback;
...
@ThreadSafe
public class KotlinScriptTaskCallbackContext implements Closeable {

    private static final Map<String, KotlinScriptTaskCallbackContext> UUID_CONTEXT = ILockCollectionFactory
            .getInstance(true)
            .newConcurrentMap();
    ...
    public KotlinScriptTaskCallbackContext(final IScriptTaskCallback callback) {
        this.uuid = UUIDs.newPseudoRandomUUID();
        this.callback = callback;
        UUID_CONTEXT.put(uuid, this);
    }

    public static KotlinScriptTaskCallbackContext getContext(final String uuid) {
        return UUID_CONTEXT.get(uuid);
    }

    public void init(final IScriptTaskEngine engine) {
        engine.getInputs().putString("kotlinScriptTaskCallbackContextUuid", getUuid());
        engine.eval(new ClassPathResource(KotlinScriptTaskCallbackContext.class.getSimpleName() + ".kts",
                KotlinScriptTaskCallbackContext.class));
    }
