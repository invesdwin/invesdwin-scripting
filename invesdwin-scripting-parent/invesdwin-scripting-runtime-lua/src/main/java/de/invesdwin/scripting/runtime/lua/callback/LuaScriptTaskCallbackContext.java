...
@ThreadSafe
public class LuaScriptTaskCallbackContext implements Closeable {

    private static final Map<String, LuaScriptTaskCallbackContext> UUID_CONTEXT = ILockCollectionFactory
            .getInstance(true)
            .newConcurrentMap();
    ...
    public LuaScriptTaskCallbackContext(final IScriptTaskCallback callback) {
        this.uuid = UUIDs.newPseudoRandomUUID();
        this.callback = callback;
        UUID_CONTEXT.put(uuid, this);
    }

    public static LuaScriptTaskCallbackContext getContext(final String uuid) {
        return UUID_CONTEXT.get(uuid);
    }

    public void init(final IScriptTaskEngine engine) {
        engine.getInputs().putString("luaScriptTaskCallbackContextUuid", getUuid());
        engine.eval(new ClassPathResource(LuaScriptTaskCallbackContext.class.getSimpleName() + ".lua",
                LuaScriptTaskCallbackContext.class));
    }
