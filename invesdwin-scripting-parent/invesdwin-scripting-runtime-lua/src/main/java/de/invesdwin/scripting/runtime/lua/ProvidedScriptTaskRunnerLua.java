...
@Immutable
@Named
public final class ProvidedScriptTaskRunnerLua
        implements IScriptTaskRunnerLua, FactoryBean<ProvidedScriptTaskRunnerLua> {

    public static final String PROVIDED_INSTANCE_KEY = IScriptTaskRunnerLua.class.getName();

    public static final ProvidedScriptTaskRunnerLua INSTANCE = new ProvidedScriptTaskRunnerLua();

    @GuardedBy("this.class")
    private static IScriptTaskRunnerLua providedInstance;

    private ProvidedScriptTaskRunnerLua() {}

    public static synchronized IScriptTaskRunnerLua getProvidedInstance() {
        if (providedInstance == null) {
            ...
            if (systemProperties.containsValue(PROVIDED_INSTANCE_KEY)) {
                try {
                    ...
                    return (IScriptTaskRunnerLua) Reflections.classForName(runner).newInstance();
                } catch (InstantiationException | IllegalAccessException e) {
                    throw new RuntimeException(e);
                }
            } else {
                final Map<String, IScriptTaskRunnerLua> runners = ILockCollectionFactory.getInstance(false)
                        .newLinkedMap();
                for (final IScriptTaskRunnerLua runner : ServiceLoader.load(IScriptTaskRunnerLua.class)) {
                    final IScriptTaskRunnerLua existing = runners.put(runner.getClass().getName(), runner);
                    if (existing != null) {
    ...
    public static synchronized void setProvidedInstance(final IScriptTaskRunnerLua providedInstance) {
        ProvidedScriptTaskRunnerLua.providedInstance = providedInstance;
        final SystemProperties systemProperties = new SystemProperties();
        if (providedInstance == null) {
    ...
    public Class<?> getObjectType() {
        return ProvidedScriptTaskRunnerLua.class;
    }
    ...
    @Override
    public ProvidedScriptTaskRunnerLua getObject() throws Exception {
        return INSTANCE;
    }
