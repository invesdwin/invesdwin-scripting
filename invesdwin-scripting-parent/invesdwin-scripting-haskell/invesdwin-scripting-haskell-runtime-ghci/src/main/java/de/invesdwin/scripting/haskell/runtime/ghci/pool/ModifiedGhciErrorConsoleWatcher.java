...
@ThreadSafe
public class ModifiedGhciErrorConsoleWatcher implements Closeable {
    ...
    public ModifiedGhciErrorConsoleWatcher(final Process process) {
                ...
                try {
                    while (!Threads.isInterrupted() && errorThread == currentThread) {
                        ...
                        if (Strings.isNotBlank(s) && !s.contains("Info: Precompiling ")) {
                            IScriptTaskRunnerHaskell.LOG.warn("< %s", s);
