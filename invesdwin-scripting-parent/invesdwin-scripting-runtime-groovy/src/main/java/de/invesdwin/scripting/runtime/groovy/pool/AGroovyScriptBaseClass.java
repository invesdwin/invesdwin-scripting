...
import de.invesdwin.scripting.runtime.groovy.callback.GroovyScriptTaskCallbackContext;
...
public abstract class AGroovyScriptBaseClass extends Script {
    ...
    public <T> T callback(final String methodName) {
        return callback(methodName, Objects.EMPTY_ARRAY);
    }

    public <T> T callback(final String methodName, final Object parameter1) {
        return callback(methodName, new Object[] { parameter1 });
    }

    public <T> T callback(final String methodName, final Object parameter1, final Object parameter2) {
    ...
    public <T> T callback(final String methodName, final Object... parameters) {
        if (!getBinding().hasVariable("groovyScriptTaskCallbackContext")) {
            if (getBinding().hasVariable("groovyScriptTaskCallbackContextUuid")) {
                ...
                getBinding().setVariable("groovyScriptTaskCallbackContext",
                        de.invesdwin.scripting.runtime.groovy.callback.GroovyScriptTaskCallbackContext
        ...
        if (returnValue.isReturnExpression()) {
            return (T) Eval.me((String) returnValue.getReturnValue());
        } else {
