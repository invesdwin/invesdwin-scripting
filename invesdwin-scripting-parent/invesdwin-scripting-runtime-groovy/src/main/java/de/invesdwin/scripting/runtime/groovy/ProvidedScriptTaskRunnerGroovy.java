...
@Immutable
@Named
public final class ProvidedScriptTaskRunnerGroovy
        implements IScriptTaskRunnerGroovy, FactoryBean<ProvidedScriptTaskRunnerGroovy> {

    public static final String PROVIDED_INSTANCE_KEY = IScriptTaskRunnerGroovy.class.getName();
    ...
    public static synchronized IScriptTaskRunnerGroovy getProvidedInstance() {
        if (providedInstance == null) {
            ...
            if (systemProperties.containsValue(PROVIDED_INSTANCE_KEY)) {
                try {
                    ...
                    return (IScriptTaskRunnerGroovy) Reflections.classForName(runner).newInstance();
                } catch (InstantiationException | IllegalAccessException e) {
                    throw new RuntimeException(e);
                }
            } else {
                final Map<String, IScriptTaskRunnerGroovy> runners = new LinkedHashMap<String, IScriptTaskRunnerGroovy>();
                for (final IScriptTaskRunnerGroovy runner : ServiceLoader.load(IScriptTaskRunnerGroovy.class)) {
                    final IScriptTaskRunnerGroovy existing = runners.put(runner.getClass().getName(), runner);
                    if (existing != null) {
    ...
    public static synchronized void setProvidedInstance(final IScriptTaskRunnerGroovy providedInstance) {
        ProvidedScriptTaskRunnerGroovy.providedInstance = providedInstance;
        final SystemProperties systemProperties = new SystemProperties();
        if (providedInstance == null) {
    ...
    public Class<?> getObjectType() {
        return ProvidedScriptTaskRunnerGroovy.class;
    }
    ...
    @Override
    public ProvidedScriptTaskRunnerGroovy getObject() throws Exception {
        return INSTANCE;
    }
