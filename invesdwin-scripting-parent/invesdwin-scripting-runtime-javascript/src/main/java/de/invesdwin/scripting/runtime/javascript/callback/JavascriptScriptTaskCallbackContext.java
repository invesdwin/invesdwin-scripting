package de.invesdwin.scripting.runtime.javascript.callback;
...
@ThreadSafe
public class JavascriptScriptTaskCallbackContext implements Closeable {

    private static final Map<String, JavascriptScriptTaskCallbackContext> UUID_CONTEXT = ILockCollectionFactory
            .getInstance(true)
            .newConcurrentMap();
    ...
    public JavascriptScriptTaskCallbackContext(final IScriptTaskCallback callback) {
        this.uuid = UUIDs.newPseudoRandomUUID();
        this.callback = callback;
        UUID_CONTEXT.put(uuid, this);
    }

    public static JavascriptScriptTaskCallbackContext getContext(final String uuid) {
        return UUID_CONTEXT.get(uuid);
    }

    public void init(final IScriptTaskEngine engine) {
        engine.eval(new ClassPathResource(JavascriptScriptTaskCallbackContext.class.getSimpleName() + ".js",
                JavascriptScriptTaskCallbackContext.class));
        engine.getInputs().putString("javascriptScriptTaskCallbackContextUuid", getUuid());
    }
